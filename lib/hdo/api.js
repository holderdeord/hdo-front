var config                  = require('../../config'),
    client                  = require('../hal/client')(config.get('api'), {cache: config.get('cache') === 'enabled'}),
...
module.exports.issues = {
    ...
    findBySlug: function (slug) {
        ...
        request.follow('issues').follow('find', {id: id}).follow('promises', null, {paginate: true});

        return request.get().then(function (response) {
            var issue    = response.embedded('issues').embedded('find'),
                promises = issue.embedded('promises').embeddedArray('promises');

            return issuePresenter.presentIssueWithPromises(issue, promises);
        });
    }
};
